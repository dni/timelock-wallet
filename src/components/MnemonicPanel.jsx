...
export default function MnemonicPanel(props) {
    ...
    const wallet = createMemo(() => {
        if (!props.mnemonic)
            return null;
        try {
            return deriveWallet(props.mnemonic, props.passphrase);
        }
        catch {
            return null;
        }
    ...
    const addresses = createMemo(() => {
        if (!props.mnemonic)
            return [];
        try {
            return deriveStandardAddresses(props.mnemonic, props.passphrase, 10);
        }
        catch {
            return [];
        }
      ...
      <Show when={props.mnemonic}>
        <div class="section passphrase-section">
          <label class="passphrase-label">
            ...
            <input type="password" value={props.passphrase} onInput={(e) => props.onPassphraseChange(e.currentTarget.value)} placeholder="(empty = no passphrase)" class="passphrase-input"/>
          </label>
        </div>
