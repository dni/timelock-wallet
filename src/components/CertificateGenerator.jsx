...
import { generateCertKeypair, signCertificate, currentBlockPeriod, periodToApproxDate, } from '../lib/certificate';
...
export default function CertificateGenerator(props) {
    ...
    const [cert, setCert] = createSignal(null);
    ...
    function handleSign() {
        ...
        try {
            const result = signCertificate(props.mnemonic, props.passphrase, props.bond.index, pubkey, certPrivkeyHex().trim(), certExpiry());
            setCert(result);
            setError('');
        }
        catch (e) {
    ...
    return (<div class="cert-generator">
      ...
      <div class="cert-body">
        <div class="cert-step">
          ...
          <div class="copy-field" style={{ 'margin-top': '0.75rem' }}>
            ...
            <input class="hex-input mono" type="text" placeholder="033a…66 hex chars (compressed pubkey)" value={certPubkeyHex()} onInput={(e) => { setCertPubkeyHex(e.currentTarget.value); setCert(null); }}/>
          </div>
        </div>
        ...
        <div class="cert-step">
          ...
          <div class="expiry-row">
            <label class="year-label">
              Block period
              <input class="year-input" type="number" min={1} value={certExpiry()} onInput={(e) => { setCertExpiry(parseInt(e.currentTarget.value) || defaultPeriod); setCert(null); }}/>
            </label>
