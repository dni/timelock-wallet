...
function bitcoinMessageHash(message) {
    ...
    const payload = concatBytes(new Uint8Array([BITCOIN_MSG_MAGIC.length]), BITCOIN_MSG_MAGIC, compactSize(msgBytes.length), msgBytes);
    return sha256(sha256(payload));
}
export function buildCertMessage(certPubkeyHex, certExpiry) {
    return `fidelity-bond-cert|${certPubkeyHex}|${certExpiry}`;
}
export function generateCertKeypair() {
    const priv = secp256k1.utils.randomPrivateKey();
    const pub = secp256k1.getPublicKey(priv, true);
    return { privkeyHex: bytesToHex(priv), pubkeyHex: bytesToHex(pub) };
}
export function currentBlockPeriod() {
...
export function periodToApproxDate(period) {
    const blockHeight = period * 2016;
    const ms = GENESIS_MS + blockHeight * 10 * 60 * 1000;
    return new Date(ms).toISOString().slice(0, 10);
}
export function signCertificate(mnemonic, passphrase, bondIndex, certPubkeyHex, certPrivkeyHex, certExpiry) {
